<template>
	<ui-form :model="params.data" :transfer-model.sync="submitData" v-slot="{data}">
		...
		<ui-text v-model="data.millName" label="礼包名称" submitfield="name" required/>
		<ui-template>
...
export default {
	...
	methods:{
		submit(){
			this.$axios.request({url:'/admin/CoinMill/setMillName',data:this.submitData,method:this.params.type,urlencode:true})
			.then(data=>{
				this.close();
			}).end();
		},
