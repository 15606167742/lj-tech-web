<template>
	<div class="com-list-content">
		<com-search :model="search" :transfer-model.sync="condition" v-slot="{data}" :table="$refs.table">
			...
			<ui-select v-model="data.typeId" label="类别" url="/admin/healthy/getType" value-field="id" text-field="typeName"/>
			<ui-template>
				<ui-button type="search"></ui-button>
				<ui-button type="add" @click="add">增加</ui-button>
			</ui-template>
		</com-search>
		<com-table ref="table" url="/admin/healthy/queryContent" :search="condition" v-slot="{data}" autolimitwidth>
			<ui-text v-model="data.title" label="标题" />
			<ui-text v-model="data.author" label="作者" />
			<ui-text v-model="data.description" label="描述" />
			<ui-image v-model="data.picUrl" label="图片" />
...
export default {
	...
	methods:{
		add(){
			this.$dialog('healthy_consult_detail',{type:'add',data:{}},()=>{
				this.$refs.table.refresh();
			})
		},
		edit(data){
			this.$dialog('healthy_consult_detail',{type:'edit',data:data},()=>{
				this.$refs.table.refresh();
			})
		},
		remove(data){
			this.$axios.delete('/admin/healthy/deleteContent?id='+data.id)
			.then(data=>{
				this.$refs.table.refresh();
			}).end();
		}
