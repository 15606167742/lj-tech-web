<template>
	<ui-form :model="info" :transfer-model.sync="submitData" v-slot="{data}" :view="edited?undefined:''" style="width: 600px;">
		<ui-textarea v-model="data.content" label="公告" :columns="2" :maxlength="250"/>
		<ui-template>
...
export default {
	...
	methods:{
		getInfo() {
			this.$axios.request({url:'/admin/announce/show?type=2', method: 'get'})
			.then(data=>{
		...
		submit(){
			this.$axios.request({url:'/admin/announce/editAnnounce?type=2',data:this.submitData,method:'post',urlencode:true})
			.then(data=>{
				this.getInfo();
			}).end();
		}
