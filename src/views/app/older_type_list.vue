<template>
	<div class="com-list-content">
		...
		<com-table ref="table" url="/admin/wisdomendowment/getTypeList" :search="condition" v-slot="{data}">
...
export default {
	...
	methods:{
		add(){
			this.$dialog('older_type_detail',{type:'add',data:{}},()=>{
				this.$refs.table.refresh();
			})
		},
		edit(data){
			this.$dialog('older_type_detail',{type:'edit',data:data},()=>{
				this.$refs.table.refresh();
			})
		},
		remove(data){
			this.$axios.delete('/admin/wisdomendowment/deleteType?id='+data.id)
			.then(data=>{
				this.$refs.table.refresh();
			}).end();
		}
