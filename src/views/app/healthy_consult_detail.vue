<template>
	<ui-form :model="params.data" :transfer-model.sync="submitData" v-slot="{data}">
		...
		<ui-template label="图片" :columns="1" required>
			...
			<ui-image v-model="data.picUrl" required cropper :cropperOption="{fixedNumber: [1.3,1]}"/>
		</ui-template>
		<ui-image v-model="data.headPicture" label="头像" />
		<ui-text v-model="data.description" label="描述" required/>
		<ui-select v-model="data.typeId" label="类别" url="/admin/healthy/getType" value-field="id" text-field="typeName" required/>
	...
	methods:{
		submit(){
			...
			if (this.params.type=='add') {
				url = '/admin/healthy/addContent';
			} else{
				url = '/admin/healthy/updateContent';
			}
