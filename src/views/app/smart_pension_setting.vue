<template>
	<ui-form :model="info" :transfer-model.sync="submitData" v-slot="{data}" :view="edited?undefined:''" style="width: 600px;">
		...
		<ui-richtext v-model="data.content" label="内容" :columns="2" required/>
		<ui-template>
...
export default {
	...
	methods:{
		getInfo() {
			this.$axios.request({url:'/admin/healthy/findHealthGuide', method: 'get'})
			.then(data=>{
				this.info = data;
				this.edited = false;
			}).end();
		...
		submit(){
			this.$axios.request({url:'/admin/healthy/updateHealthGuide',data:this.submitData,method:'put',urlencode:true})
			.then(data=>{
				this.getInfo();
			}).end();
		}
