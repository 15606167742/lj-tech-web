<template>
	<ui-form :model="model" v-slot="{data}" view>
		...
		<ui-select v-model="data.typeId" label="任务分类" url="/admin/taskType" value-field="id" text-field="typeName" multiple/>
...
export default {
	...
	methods:{
		getDetail(){
			this.$axios.request({url:'/admin/task/'+this.params.data.taskId, method: 'get'})
			.then(data=>{
				this.model = data;
			}).end();
		},
