<template>
	<ui-validate :value="errorText" @enter="$emit('enter',$event)" :novalidate="novalidate" :nomessage="nomessage" :verifying="verifying" v-if="!view">
		<el-input clearable :show-password="showPassword" :show-word-limit="showWordLimit" :maxlength="maxlength" :disabled="disabled" :type="type" rows="4" :value="value" @change="$emit('change',$event)" @input="$emit('input',$event)" @blur="validate" :placeholder="'请输入'+description" />
	</ui-validate>
