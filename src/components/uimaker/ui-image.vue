	...
	<div class="uimaker-image" :class="(size?('uimaker-image-'+size):'')+' '+(fit?('uimaker-image-'+fit):'')">
		<ui-validate :value="errorText" :novalidate="novalidate" :nomessage="nomessage" :verifying="verifying" v-if="!view">
			<el-upload class="avatar-uploader" accept="image/*" :show-file-list="multiple?true:false" :action="action" :on-success="onSuccessEvent" :on-preview="onPreviewEvent" :before-upload="beforeUploadEvent" 
				...
				:auto-upload="!cropper" :on-change='changeUpload' :limit="limit">
		...
		<el-dialog :visible.sync="cropperVisible" title="裁剪" custom-class="uimaker-preview-dialog" width="800px" top="50vh" append-to-body>
			<div style="width: 100%;height: 600px;position: relative;">
				<vue-cropper ref="cropper" :img="selfCropperOption.img" :autoCrop="selfCropperOption.autoCrop" :centerBox="selfCropperOption.centerBox" :fixed="selfCropperOption.fixed" :fixedNumber="selfCropperOption.fixedNumber"></vue-cropper>
				<el-button style="position: absolute;right: 10px;bottom: 10px;" type="success" icon="el-icon-check" circle @click="finish"></el-button>
			</div>
		</el-dialog>
	</div>
</template>
...
export default {
	...
	props:{
		...
		cropperOption:{type:Object,default:()=>{return {}}},
		limit:{type:Number}
	},
	data(){
		return {
	...
	methods:{
		...
		changeUpload(file){
			if(this.cropper){
				...
				fr.onload = () =>{
					this.selfCropperOption = Object.assign(this.selfCropperOption, this.cropperOption)
					this.selfCropperOption.img = fr.result;
		...
		finish(){
			...
			this.$refs.cropper.getCropBlob((data) => {
				let newFile = new window.File([data], this.fileObj.raw.name, {type: this.fileObj.raw.type});
				let form = new FormData();
				form.append('file', newFile);
				this.$axios.request({url: WEBCONFIG.resourcePath+this.uploadPath,headers:{'Content-Type': 'multipart/form-data'},method: 'post',data:form}).then(data=>{
					let value='';
					if(this.multiple){
						if(this.fileList.length>0){
							value=this.value+','+this.getResponseValue({data});
						}else{
							value=this.getResponseValue({data});
						}
					}
					else{
						value=this.getResponseValue({data});
